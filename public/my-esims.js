...
async function loadOrdersFromServer(userId) {
    try {
        ...
        const response = await fetch(`/api/orders?telegram_user_id=${userIdStr}`);
        if (!response.ok) {
        ...
        if (result.success && result.data && Array.isArray(result.data)) {
            console.log(`✅ Loaded ${result.data.length} orders from server for user ${userIdStr}`);
            ...
            console.log('No orders found or invalid response format:', result);
        }
    } catch (error) {
