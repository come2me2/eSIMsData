    ...
    if (tg && tg.BackButton) {
        ...
        tg.BackButton.onClick(() => {
            ...
            tg.HapticFeedback.impactOccurred('light');
                    ...
                    // Переходим обратно на главную страницу с активной вкладкой Region
                    try {
                        window.location.href = 'index.html?segment=region';
                    } catch (e) {
                        console.error('❌ Region: Ошибка при переходе на главную', e);
...
async function loadPlansFromAPI(regionName, useCache = true) {
    console.log('🔵 loadPlansFromAPI called with region:', regionName, 'useCache:', useCache);
        ...
        try {
            ...
            if (cachedData && cacheTimestamp) {
                ...
                if (cacheAge < CACHE_TTL) {
                    console.log(`✅ Loading ${regionName} plans from localStorage cache (instant load)`);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupNavigation(); // Настраиваем нижнее меню
