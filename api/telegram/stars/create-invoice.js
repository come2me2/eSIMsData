 ...
 *
 * Формула расчета:
 * Stars = (price / (1 - telegram_fee) / stars_rate)
 *
 * Пример для esim_1GB_7D_AE_V2:
 * - price = $2.26 (себестоимость)
 * - telegram_fee = 0.25 (25%)
 * - stars_rate = 0.013 ($0.013 за 1 Star)
 * - Stars = (2.26 / (1 - 0.25) / 0.013) = (2.26 / 0.75 / 0.013) ≈ 232 Stars
 *
 * ⚠️ ВАЖНО: 
 * - price должна быть СЕБЕСТОИМОСТЬЮ (cost), а не финальной ценой!
 * - Маржа уже учтена в финальной цене для пользователя, не нужно учитывать в расчете Stars
 */
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // Формула расчета Stars с учетом комиссии Telegram:
        // Stars = (price / (1 - telegram_fee) / stars_rate)
        // 
        // Где:
        ...
        //
        // Пример: (2.26 / (1 - 0.25) / 0.013) = (2.26 / 0.75 / 0.013) ≈ 232 Stars
        //
        // ⚠️ ВАЖНО: 
        // - price должна быть СЕБЕСТОИМОСТЬЮ (cost), а не финальной ценой!
        // - Маржа уже учтена в финальной цене для пользователя, не нужно учитывать в расчете Stars
        // - Комиссия Telegram (25%) учитывается в формуле
        const amountStars = Math.max(
            MIN_STARS,
            Math.ceil(
                priceNumber / (1 - STARS_TELEGRAM_FEE) / STARS_RATE
            )
        );
        ...
        console.log(`💰 Stars calculation:`, {
            ...
            formula: `(${priceNumber} / (1 - ${STARS_TELEGRAM_FEE}) / ${STARS_RATE})`
        });
