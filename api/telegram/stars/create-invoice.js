...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // Используем setImmediate для выполнения после отправки ответа
        if (invoiceId) {
            setImmediate(async () => {
                try {
                    // Небольшая задержка для гарантии, что ответ уже отправлен
                    ...
                    const orderReq = {
                        ...
                        body: {
                            ...
                            orderReference: `pending_${invoiceId}`, // Временный ID, будет заменен после оплаты
                            status: 'on_hold',
                    ...
                    console.log('✅ Order created with status on_hold (async):', {
                        invoiceId,
                        telegram_user_id,
                        bundle_name,
                    ...
                    // Не критично, заказ можно создать позже при оплате
                }
            });
        }

        // Отправляем ответ сразу, не дожидаясь создания заказа on_hold
